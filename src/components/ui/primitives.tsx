...
export function FilteredEmptyState({ onClear }: { onClear: () => void }) {
  return (
    <div className="flex flex-col items-start gap-3 rounded-[--radius-card] border border-dashed border-line px-6 py-10">
      <h3 className="type-heading text-ink">Nothing matches those filters</h3>
      <p className="type-body text-ink-soft">
        The library is not empty — this combination of filters is. Clear them to see everything.
      </p>
...
export function FailedState({
  ...
  return (
    ...
    >
      ...
      <p className="type-body max-w-prose text-ink-soft">{body}</p>
      {hint ? <div className="type-small max-w-prose text-ink-muted">{hint}</div> : null}
