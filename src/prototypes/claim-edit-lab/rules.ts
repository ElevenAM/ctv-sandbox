...
export const FIELDS = {
  ...
  | { label: string; kind: "enum"; options: readonly string[] }
...
export const describe = (c: Condition) =>
  `${FIELDS[c.field].label} ${OPERATORS[c.operator]} ${c.value}`;
...
const isUsable = (c: Condition) =>
  FIELDS[c.field].kind === "enum" ||
  (c.value.trim() !== "" && Number.isFinite(Number(c.value)));
...
export function evaluate(rule: Rule, lines: readonly ClaimLine[]): Evaluation {
  ...
  for (const line of lines) {
    ...
    if (rule.action.kind === "cap") {
      ...
      savings = Math.round(
        (line.paid * (line.units - rule.action.maxUnits)) / line.units,
      );
    }
...
export function toSpec(rule: Rule): string {
  ...
  const then =
    rule.action.kind === "deny"
      ? "deny the line"
      : `pay at most ${rule.action.maxUnits} units`;
